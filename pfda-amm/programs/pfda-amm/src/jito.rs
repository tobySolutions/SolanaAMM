/// Jito auction integration for PFDA batch clearing.
///
/// How it works:
...
///   4. On-chain: ClearBatch validates that the SOL transfer (bid) happened
///      by checking the treasury account's lamport balance increased
///
/// The on-chain program enforces:
///   - Minimum bid amount
///   - Bid recipient is the protocol treasury
///   - Revenue split between protocol and LP
///
/// The Jito tip is separate — it goes to validators and is not verified on-chain.
/// What we verify is the PROTOCOL BID (SOL paid to our treasury).

/// Revenue distribution from searcher bid.
///
/// From the spec:
///   Π_gross = total searcher payment to protocol
///   protocol_share = α * Π_gross (α = auction competitiveness, default 50%)
///   lp_share = (1-α) * Π_gross
///
/// The searcher separately pays Jito tips to validators — that's not our concern.
pub fn compute_bid_split(
    bid_lamports: u64,
    alpha_bps: u16,
) -> (u64, u64) {
